...
async def startupmessage():
    ...
    try:
        if BOTLOG:
            Config.CATUBLOGO = await jepiq.tgbot.send_file(
                BOTLOG_CHATID,
                "https://telegra.ph/file/e1de4cd3bee3a4eb5b098.jpg",
               caption="⌯︙**بــوت بابلو يـعـمـل بـنـجـاح**  ✅ \n⌯︙**قـنـاة الـسـورس**  :  @kkjtt",
                buttons=[(Button.url("كروب بابلو", "https://t.me/kkjtt"),)],
            )
    except Exception as e:
        LOGS.error(e)
        return None
    ...
    try:
        if msg_details:
            ...
            text = (
                message.text
                + "\n\n**⌯︙اهلا وسهلا لقد قمت باعاده تشغيل بـوت بابلو تمت بنجاح**"
            )
...
jepthon = {"@KKJTT", "@KKJTT", "@Story_pablo2620"}
async def saves():
...
async def verifyLoggerGroup():
    ...
    if BOTLOG:
        ...
        descript = "- عزيزي المستخدم هذه هي مجموعه الاشعارات يرجى عدم حذفها  - @KKJTTn"
        photobt = await jepiq.upload_file(file="JepIQ/razan/resources/start/Jepthon.jpg")
        _, groupid = await create_supergroup(
            "مجموعة اشعارات بابلو ", jepiq, Config.TG_BOT_USERNAME, descript, photobt
        )
    ...
    if PM_LOGGER_GROUP_ID != -100:
        ...
        descript = "⌯︙ وظيفه الكروب يحفظ رسائل الخاص اذا ما تريد  احذف الكروب نهائي \n  - @KKJTT"
